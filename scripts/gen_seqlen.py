...
for key in seq_keys:
    if key in raw:
        seeds = [raw[key][s] for s in raw[key] if isinstance(raw[key][s], dict) and 'mse' in raw[key][s]]
        if seeds:
            humanoid_data.append({
                'mse': np.mean([s['mse'] for s in seeds]),
                'r2': np.mean([s['r2'] for s in seeds])
            })
# Use same data for ant (placeholder)
ant_data = humanoid_data
results = {'humanoid': humanoid_data, 'ant': ant_data}
