...
def train_eval(model, Xs, Xa, Y, Xv, Xav, Yv, seed=SEED, use_physics_loss=False):
    ...
    for ep in range(EPOCHS):
        ...
        for i in range(0, len(idx), BS):
            ...
            if use_physics_loss and hasattr(model, 'physics_loss'):
                loss = loss + model.physics_loss(s_batch, pred, a_batch)
...
class SparseSSM(nn.Module):
    """稀疏交互SSM - 学习哪些维度互相影响"""
    def __init__(self, state_dim, action_dim, d_model=96, d_state=16, n_layers=2, sparsity=0.5):
        super().__init__()
        self.encoder = nn.Sequential(nn.Linear(state_dim + action_dim, d_model), nn.GELU(), nn.Linear(d_model, d_model))

        # 稀疏掩码：学习哪些维度互相影响
        self.sparse_mask = nn.Parameter(torch.ones(d_state, d_state) * 0.5)
    ...
    def physics_loss(self, states, pred, actions):
        """稀疏性约束：鼓励稀疏交互"""
        mask = torch.sigmoid(self.sparse_mask)
        sparsity_loss = torch.mean(mask)  # 鼓励mask接近0
        return 0.01 * sparsity_loss

# ============================================================
# 创新2: 物理约束SSM
# ============================================================
class PhysicsConstrainedSSM(nn.Module):
    """物理约束SSM - 嵌入能量守恒和平滑性"""
    def __init__(self, state_dim, action_dim, d_model=96, d_state=16, n_layers=2):
        super().__init__()
        self.encoder = nn.Sequential(nn.Linear(state_dim + action_dim, d_model), nn.GELU(), nn.Linear(d_model, d_model))
    ...
    def physics_loss(self, states, pred, actions):
        ...
        energy_after = self.energy_estimator(pred)
        energy_loss = torch.mean((energy_after - energy_before) ** 2)

        # 平滑性：状态变化应该连续
        if states.shape[1] > 1:
            velocity = states[:, 1:, :] - states[:, :-1, :]
            pred_velocity = pred - states[:, -1, :]
            smooth_loss = torch.mean((pred_velocity - velocity[:, -1, :]) ** 2)
        else:
            smooth_loss = torch.tensor(0.0, device=states.device)

        return 0.1 * energy_loss + 0.01 * smooth_loss
...
class AdaptiveSSM(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, d_model=96, d_state=16, n_layers=2):
        ...
        self.param_generator = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(d_model // 4, d_state * 2)  # 生成A和B的调整
        )
    ...
    def forward(self, states, actions):
        ...
        # 生成自适应参数
        param_adjust = self.param_generator(h[:, -1, :])  # (B, d_state*2)
...
if __name__ == '__main__':
    ...
    RESULTS_FILE = 'experiments/physics_ssm.json'
    ...
    configs = {
        'SimpleSSM': (lambda: SimpleSSM(348, 17), False),
        'SparseSSM': (lambda: SparseSSM(348, 17), True),
        'PhysicsConstrainedSSM': (lambda: PhysicsConstrainedSSM(348, 17), True),
        'AdaptiveSSM': (lambda: AdaptiveSSM(348, 17), False),
    }

    print('\n' + '='*60, flush=True)
    print('物理约束SSM探索', flush=True)
    print('='*60, flush=True)

    for name, (model_fn, use_physics) in configs.items():
        if name in results:
        ...
        print(f'\n{name}:', flush=True)
        model = model_fn()
        r = train_eval(model, Xs, Xa, Y, Xv, Xav, Yv, use_physics_loss=use_physics)
        results[name] = r
