...
该模型利用对角SSM的FFT卷积结构对机器人历史状态与动作序列进行高效建模,
在保持较低计算复杂度($O(T\log T)$)的同时实现对未来状态轨迹的准确预测.
在此基础上, 将SSM-WM嵌入模型预测控制(MPC)框架,
实现面向人形机器人运动规划的在线决策.
在合成机器人数据集上的实验结果表明,
所提方法在推理速度上较LSTM世界模型提升约7倍(3.8ms vs 27.8ms),
...
This paper proposes a lightweight world model based on diagonal state space models (SSM-WM) for humanoid robot state prediction in high-dimensional nonlinear environments. The proposed model leverages the FFT convolution structure of diagonal SSM to efficiently encode historical states and action sequences, achieving accurate future state trajectory prediction with $O(T\log T)$ computational complexity. The SSM-WM is further integrated into a model predictive control (MPC) framework for online motion planning. Experiments on synthetic robot datasets demonstrate that the proposed method achieves 7x faster inference than LSTM world models (3.8ms vs 27.8ms) with 17\% fewer parameters, meeting the $<$10ms real-time control requirement, validating its effectiveness and practicality for embodied intelligence.
...
本文的主要贡献如下:
\begin{enumerate}
\item[1)] 提出一种基于对角SSM的轻量级世界模型(SSM-WM), 利用FFT卷积结构对机器人状态-动作序列进行高效建模, 实现$O(T\log T)$的计算复杂度;
\item[2)] 将SSM-WM嵌入MPC框架, 实现面向人形机器人运动规划的在线决策;
\item[3)] 在合成机器人数据集上验证了所提方法的有效性, SSM-WM在推理速度上较LSTM提升约7倍, 满足实时控制要求.
\end{enumerate}
...
\textbf{SSM主干网络.}
采用多层SSM块对编码后的序列进行建模.
每个SSM块包含对角SSM(diagonal SSM)层和门控机制.
对角SSM将状态矩阵${\bm A}$约束为对角矩阵, 使得SSM的递推可以通过FFT卷积高效计算.
具体而言, 对角SSM的卷积核为:
\begin{eqnarray}
...
每个SSM块还包含门控机制:
\begin{eqnarray}
{\bm g} = \sigma({\bm W}_g{\bm z}_t),\quad
{\bm y}_t = {\bm g} \odot \text{SSM}({\bm z}_t) + (1-{\bm g}) \odot {\bm z}_t,
\end{eqnarray}
...
为验证各组件的有效性, 在$T=32$的设置下进行了以下消融实验, 结果如表~2所示.

\begin{center}
{\CJKfamily{kai} 表~2~~消融实验结果($T=32$)\\
Table~2~~Ablation study results ($T=32$)}\\
\label{tab:2} \vskip 3pt
{\fontsize{9.3pt}{11.6pt}\selectfont
\begin{tabular}{@{ }lcc@{ }}\toprule
配置 & MSE & 相对变化\\
\midrule
SSM-WM(完整) & 0.004455 & 基线\\
去除门控机制 & 0.004566 & +2.5\%\\
去除残差连接 & 0.004525 & +1.6\%\\
\bottomrule
\end{tabular}}\end{center}
...
实验结果表明:
\begin{enumerate}
\item[1)] 门控机制对模型性能贡献最大(2.5\%), 说明门控能够有效控制信息流, 选择性地保留有用信息;
\item[2)] 残差连接贡献1.6\%, 虽然影响较小, 但有助于加速收敛和稳定训练;
\item[3)] 各组件均对最终性能有正向贡献, 验证了架构设计的合理性.
\end{enumerate}
...
\bibitem{9}GU A, GU A, GU A, et al. HiPPO: Recurrent memory with optimal polynomial projections. {\it Advances in Neural Information Processing Systems}, 2020, 33: 1474--1487.
