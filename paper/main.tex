...
\end{eqnarray}
因$\bar{{\bm A}}$对角, 故$\bar{{\bm A}}^{t-\tau} = \diag(\bar{a}_n^{t-\tau})$, 代入得
\begin{eqnarray}
{\bm C}\bar{{\bm A}}^{t-\tau}\bar{{\bm B}} = \sum_n C_n\bar{a}_n^{t-\tau}\bar{b}_n = K[t{-}\tau].
\end{eqnarray}
因此$y_t = \sum_{\tau=0}^{t} K[t{-}\tau]x_\tau + {\bm D}x_t$, 与卷积模式一致.
证毕.
\begin{Thm}
\label{thm:complexity}
设输入序列长度$T$, SSM隐状态维度$N$, MIMO-WM隐空间维度$D$,
MIMO块层数$L$, 状态维度$d_s$, 动作维度$d_a$.
对角SSM单层的递推模式时间复杂度为$O(TN)$, 卷积模式为$O(T\log T)$.
MIMO-WM整体时间复杂度为$O(LTD\log T + LTD^2 + TD(d_s+d_a))$.
作为对比, LSTM的时间复杂度为$O(TLD^2)$, Transformer为$O(T^2LD+TLD^2)$.
当$T > D/\log D$时, MIMO-WM在长序列下的计算效率最优.
\end{Thm}

{\CJKfamily{hei}证}~~
递推模式每步执行一次$N$维复向量点乘和一次标量乘法,
共计$T$步, 时间复杂度$O(TN)$, 空间复杂度$O(N)$.
卷积模式将序列处理转化为$T$点循环卷积, 通过FFT实现,
时间复杂度$O(T\log T)$, 空间复杂度$O(T)$.
当$N > \log_2 T$时, $T\log T < TN$, 卷积模式更高效.
对本文$T=32$, $N=16$, $\log_2 32 = 5 < 16$, 故选用卷积模式.

MIMO-WM的整体复杂度由其各模块求和得到:
编码器将$T$个$d_s+d_a$维向量映射至$D$维, 复杂度$O(TD(d_s+d_a))$;
每个MIMO块包含层归一化$O(TD)$, SSM卷积$O(TD\log T)$,
门控和输出投影$O(TD^2)$, 残差连接$O(TD)$,
$L$层总计$O(LTD\log T + LTD^2)$;
解码器复杂度$O(TDd_s)$.
总和即$O(LTD\log T + LTD^2 + TD(d_s+d_a))$.

LSTM每步需计算遗忘门、输入门、输出门和候选状态,
每组$O(D^2)$, 共$4$组, 总计$O(4TLD^2) = O(TLD^2)$.
Transformer每层的自注意力为$O(T^2D)$, 前馈网络为$O(TD^2)$.
MIMO-WM的$O(TD\log T)$在$T$较大时显著优于Transformer的$O(T^2D)$.
以本文配置$T=32$, $D=96$计算, MIMO-WM比Transformer约快6倍.
证毕.
...
收敛速率: 期望代价以指数速率收敛到全局最优,
$\mathbb{E}[J({\bm a}_{\text{best}}^{(k)})] - J^*
\leqslant \frac{\mu}{2}\|\Sigma^{(0)}\|_F^2 \exp\big(-\frac{N_e}{K}k\big)$.

分布收敛: 采样分布$\mathcal{N}({\bm \mu}^{(k)}, {\bm \Sigma}^{(k)})$弱收敛到退化分布$\delta_{{\bm a}^*}$.
\end{Thm}

{\CJKfamily{hei}证}~~
先证明方差缩减性质. 设第$k$轮采样集$\mathcal{S}^{(k)}$含$K$个独立同分布样本,
其全样本方差估计为${\bm \Sigma}_{\mathcal{S}}^{(k)} = \frac{1}{K-1}\sum_{{\bm a} \in \mathcal{S}^{(k)}}({\bm a} - \bar{{\bm \mu}})({\bm a} - \bar{{\bm \mu}})^{\rm T}$.
...
故期望代价单调非增, 即$\mathbb{E}[J({\bm a}_{\text{best}}^{(k)})] \leqslant \mathbb{E}[J({\bm a}_{\text{best}}^{(k-1)})]$.

由$\mu$-强凸性, $J({\bm a}) \geqslant J^* + \frac{\mu}{2}\|{\bm a} - {\bm a}^*\|^2$.
设$\|{\bm \Sigma}^{(0)}\|_F^2 = \sigma_0^2$, 由方差递推得
$\|{\bm \Sigma}^{(k)}\|_F^2 \leqslant \sigma_0^2 (1 - \frac{N_e}{K})^{2k} \leqslant \sigma_0^2 \exp(-2\frac{N_e}{K}k)$,
代入强凸不等式取期望即得指数收敛速率.

由于${\bm \Sigma}^{(k)} \to {\bm 0}$, ${\bm \mu}^{(k)} \to {\bm a}^*$,
$\mathcal{N}({\bm \mu}^{(k)}, {\bm \Sigma}^{(k)})$在$k\to\infty$时弱收敛到退化分布$\delta_{{\bm a}^*}$.
证毕.
