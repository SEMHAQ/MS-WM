...
\textbf{数据集.} 使用Open X-Embodiment数据集\cite{8}中的子集进行实验,
选取包含人形机器人和双臂操作任务的场景.

\textbf{基线方法.} 与以下方法进行对比:
\begin{enumerate}
\item[1)] LSTM世界模型\cite{4}: 基于LSTM的循环世界模型;
\item[2)] Transformer世界模型\cite{5}: 基于标准Transformer的世界模型;
\item[3)] S4世界模型\cite{6}: 基于S4架构的世界模型.
\end{enumerate}

\textbf{评价指标.} 采用均方误差(MSE)、推理时间和参数量作为评价指标.

\subsection{实验结果}

实验结果如表~1所示. 所提出的SSM-WM在预测精度上与Transformer世界模型相当,
但在推理速度和参数效率上具有显著优势.

\begin{center}
{\CJKfamily{kai} 表~1~~不同世界模型方法的性能对比\\
Table~1~~Performance comparison of different world model methods}\\
\label{tab:1} \vskip 3pt
{\fontsize{9.3pt}{11.6pt}\selectfont
\begin{tabular}{@{ }lccc@{ }}\toprule
方法 & MSE & 推理时间/ms & 参数量/M\\
\midrule
LSTM-WM & 0.0234 & 12.3 & 2.1\\
Transformer-WM & 0.0156 & 45.7 & 8.5\\
S4-WM & 0.0163 & 10.1 & 3.2\\
SSM-WM(本文) & 0.0152 & 9.8 & 3.4\\
\bottomrule
