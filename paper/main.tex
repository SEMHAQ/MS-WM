...
并行SSM层对输入$\tilde{{\bm z}}_t \in \mathbb{R}^D$的每个维度$d$并行运行一个独立SSM
\begin{eqnarray}
{\bm h}_t^{(d)} = \bar{{\bm A}}^{(d)}{\bm h}_{t-1}^{(d)} + \bar{{\bm B}}^{(d)}\tilde{z}_t^{(d)},
\end{eqnarray}
其中$\bar{{\bm A}}^{(d)} \in \mathbb{C}^{N \times N}$为对角状态矩阵,
${\bm h}_t^{(d)} \in \mathbb{C}^N$为第$d$维的隐状态, 各维度参数独立.

门控融合先对输入进行层归一化, 再经SSM变换和sigmoid门控:
\begin{equation}
\left\{\begin{aligned}
...
卷积模式通过核$K^{(d)}[t] = \sum_n C_n^{(d)}\bar{b}_n^{(d)}(\bar{a}_n^{(d)})^t$并行计算
\begin{equation}
...
为分析序列长度对MIMO-WM预测精度的影响, 在Humanoid数据集上测试了不同序列长度$T$下的性能, 结果如表~4所示.
...
$T$ & MSE$(\times10^{-2})$ & $R^2$ \\
\midrule
8  & 20.14$\pm$0.13 & 0.765 \\
16 & \textbf{19.23$\pm$0.14} & \textbf{0.764} \\
32 & 21.18$\pm$0.04 & 0.735 \\
64 & 21.28$\pm$0.16 & 0.708 \\
128 & 41.13$\pm$0.36 & 0.448 \\
\bottomrule
\multicolumn{3}{@{}l@{}}{\fontsize{7pt}{8.5pt}\selectfont 注: 3 seeds; Humanoid数据集.}
\end{tabular}}
\par\endgroup
\end{minipage}

表~4表明, MIMO-WM在$T{=}16$时取得最优精度, MSE为19.23. 当$T$增至32及以上时, 精度逐步下降, $T{=}128$时MSE增至41.13, R²降至0.448. 这表明对于Humanoid高维动力学系统, 较短的序列长度更有利于预测, 推荐$T \in [8, 32]$.
