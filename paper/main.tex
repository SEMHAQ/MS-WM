...
本节分析MIMO-SSM（即D通道并行对角SSM）的理论性质.
...
\textbf{递推模式}: 每个通道$d$逐步计算
\begin{eqnarray}
\label{eq:recurrent}
{\bm h}_t^{(d)} = \bar{{\bm A}}^{(d)}{\bm h}_{t-1}^{(d)} + \bar{{\bm B}}^{(d)}x_t^{(d)}, \quad
y_t^{(d)} = {\bm C}^{(d)}{\bm h}_t^{(d)} + {\bm D}x_t^{(d)}.
\end{eqnarray}

\textbf{卷积模式}: 通过核$K^{(d)}[t] = \sum_n C_n^{(d)}\bar{b}_n^{(d)}(\bar{a}_n^{(d)})^t$并行计算
\begin{eqnarray}
...
{\CJKfamily{hei}证}~~
对任意通道$d$, 由隐状态展开${\bm h}_t^{(d)} = \sum_{\tau=0}^{t} (\bar{{\bm A}}^{(d)})^{t-\tau} \bar{{\bm B}}^{(d)} x_\tau^{(d)}$,
代入输出得
$y_t^{(d)} = \sum_{\tau=0}^{t} K^{(d)}[t{-}\tau] x_\tau^{(d)} + {\bm D}x_t^{(d)}$,
...
值得注意的是, MIMO-WM的参数量仅0.138M, 为所有方法中最轻量, 显著低于Mamba-WM的0.224M.
