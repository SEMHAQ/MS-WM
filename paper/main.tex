...
本节给出MIMO-WM的理论分析, 包括SSM双模式等价性、计算复杂度和CEM-MPC收敛性.
...
递推模式式\eqref{eq:recurrent}与卷积模式式\eqref{eq:convolution}产生完全相同的输出序列.
\end{Thm}
...
{\CJKfamily{hei}证}~~
递推模式每步执行一次$N$维向量点乘和一次标量乘法, 共$T$步, 时间$O(TN)$; 仅需存储当前隐状态, 空间$O(N)$.
卷积模式通过FFT计算$T$点循环卷积, 时间$O(T\log T)$; 需存储卷积核和频域表示, 空间$O(T)$.
当$N > \log T$时, $T\log T < TN$, 卷积模式更高效.
对本文Humanoid数据集$T=32$, $N=16$, $\log_2 32 = 5 < 16$, 故卷积模式更高效. \hfill$\square$

\begin{Rem}{\fontsize{9.3pt}{11.6pt}\selectfont
在MPC部署阶段, 若仅需单步预测($T=1$), 递推模式的$O(N)$复杂度优于卷积模式,
且具有更低的实际延迟. 因此MIMO-WM在训练时使用卷积模式, 部署时可切换至递推模式.}\end{Rem}

\begin{Cor}
\label{cor:gate}
MIMO-WM的sigmoid门控机制使模型能够根据输入内容动态调节SSM输出的信息流.
设门控输出为${\bm o}_t = {\bm h}_t \odot \sigma({\bm W}_g\tilde{{\bm z}}_t + {\bm b}_g)$, 则:
当$\sigma({\bm W}_g\tilde{{\bm z}}_t + {\bm b}_g) \to {\bm 1}$时, MIMO-WM退化为无门控的SSM模型;
当$\sigma({\bm W}_g\tilde{{\bm z}}_t + {\bm b}_g) \to {\bm 0}$时, SSM分支的输出被完全抑制.
门控参数${\bm W}_g$的梯度依赖于SSM输出${\bm h}_t$, 使得门控能够学习"何时信任SSM输出".
\end{Cor}
...
预实验表明$M{=}5$轮已足够, 进一步增加迭代次数对控制精度提升有限.}\end{Rem}
