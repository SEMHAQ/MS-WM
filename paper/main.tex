...
3)~~在机器人世界模型状态预测任务上系统比较了LSTM、GRU、Transformer、Mamba、TCN和MIMO-WM六种序列模型, 揭示了门控机制对SSM表达能力的显著提升;
...
\textbf{MIMO-SSM}架构. 传统SSM为单输入单输出(SISO)结构,
每个隐藏维度独立维护一个标量状态.
...
\begin{eqnarray}
{\bm z}_t' = {\bm W}_1[{\bm s}_t; {\bm a}_t] + {\bm b}_1, \quad
...
MIMO块是MIMO-WM的核心组件. 如图~1所示, 每个MIMO块包含
\textbf{层归一化}、\textbf{并行SSM层}和\textbf{门控融合}:

\begin{enumerate}
\item \textbf{并行SSM层}: 对输入$\tilde{{\bm z}}_t \in \mathbb{R}^D$的
每个维度$d=1,\ldots,D$, 并行运行一个独立SSM:
\begin{eqnarray}
{\bm h}_t^{(d)} = \bar{{\bm A}}^{(d)}{\bm h}_{t-1}^{(d)} + \bar{{\bm B}}^{(d)}\tilde{z}_t^{(d)},
\end{eqnarray}
其中$\bar{{\bm A}}^{(d)} \in \mathbb{C}^{N \times N}$为对角矩阵,
${\bm h}_t^{(d)} \in \mathbb{C}^N$为第$d$维的隐状态.
所有维度共享相同结构但参数独立,
使得不同维度能够捕获不同类型的动力学特征.

\item \textbf{门控融合}: 通过sigmoid门控机制动态调节SSM输出的信息流:
\begin{eqnarray}
\tilde{{\bm z}}_t = \text{LayerNorm}({\bm z}_t), \quad
{\bm h}_t = \text{DiagSSM}(\tilde{{\bm z}}_t), \quad
...
\end{eqnarray}
其中${\bm W}_o \in \mathbb{R}^{D \times D}$.
\end{enumerate}

门控机制使模型能够根据当前输入动态选择保留或抑制SSM输出的每个维度,
仅需$O(D^2)$额外参数.
...
{\CJKfamily{hei}证}~~
由引理, $y_t = {\bm C}{\bm h}_t + {\bm D}x_t
= {\bm C}\sum_{\tau=0}^{t} \bar{{\bm A}}^{t-\tau}\bar{{\bm B}} x_\tau + {\bm D}x_t$.
$\bar{{\bm A}}$为对角, 故$\bar{{\bm A}}^{t-\tau} = \diag(\bar{a}_n^{t-\tau})$,
...
递推模式适合低延迟单步预测, 卷积模式适合批量训练.
MIMO-WM可在两种模式间动态切换.}\end{Rem}
...
\begin{Site}
\label{lem:elite}
设第$k$轮采$K$个样本, $E^{(k)}$为$N_e$个精英集.
则精英方差满足$\|{\bm \Sigma}^{(k+1)}\|_F \leqslant (1 - \frac{N_e}{K})\|{\bm \Sigma}^{(k)}\|_F$.
\end{Site}

{\CJKfamily{hei}证}~~
全样本方差${\bm \Sigma}_{\mathcal{S}}^{(k)} = \frac{1}{K-1}\sum({\bm a} - \bar{{\bm \mu}})({\bm a} - \bar{{\bm \mu}})^{\rm T}$.
...
\begin{Rem}{\fontsize{9.3pt}{11.6pt}\selectfont
本文$K{=}256$, $N_e{=}32$, 方差每轮缩减$1{-}32/256{=}0.875$,
$M{=}5$轮后缩小至$0.875^5{\approx}0.51$.}\end{Rem}
