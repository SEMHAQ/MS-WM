...
本文提出基于MIMO-SSM的人形机器人轻量级世界模型MIMO-WM, 采用多输入多输出状态空间模型（multi-input multi-output state space model, MIMO-SSM）架构, 通过并行扫描算法实现高效序列建模, 并引入sigmoid门控机制增强表达能力.
在Gymnasium MuJoCo基准的Humanoid(348维)和HumanoidStandup(376维)两个人形机器人数据集上的多种子实验表明:
MIMO-WM在Humanoid上取得最优精度, 均方误差（mean squared error, MSE）为$19.87\times10^{-2}$, 比Mamba-WM降低1.5\%, 比Transformer-WM降低29.3\%;
参数量仅0.138M, 为所有方法中最轻量.
理论分析证明了对角SSM递推与卷积模式的等价性以及交叉熵方法（cross-entropy method, CEM）采样式MPC的收敛性保证.
...
2)~~在Gymnasium MuJoCo基准的Humanoid(348维)和HumanoidStandup(376维)两个人形机器人数据集上进行了多种子实验. 结果表明MIMO-WM在Humanoid上取得最优精度, 均方误差（mean squared error, MSE）为$19.87\times10^{-2}$, 比Mamba-WM降低1.5\%, 比Transformer-WM降低29.3\%, 同时参数量最少;
...
4)~~理论分析证明了对角SSM递推与卷积模式的等价性(定理1)、计算复杂度优势(定理2)和交叉熵方法（cross-entropy method, CEM）采样式MPC收敛性保证(定理3).
...
其中GELU为高斯误差线性单元（Gaussian Error Linear Unit）.
\end{eqnarray}
